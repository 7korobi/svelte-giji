...
export function createEditor(configuration: SvelteEditorConfig = {}): EditorResponse {
  ...
  const { subscribe: subscribeData, set: setData, update: updateData } = writable<OutputData>(
    initialData
  )
  ...
  function editor(node: HTMLElement, parameters: SvelteEditorConfig = {}) {
    async function setup() {
      ...
      const { tools } = await import('./editorjs-tools')
