...
import { DIC, sort } from 'svelte-map-reduce-store'
...
<Portrates>
      ...
      <div>
        <Portrate face_id={o.face_id}>
          {#if show_summary(o.face_id, summaries)}
            ...
            {#if 'date_min' === order}
              <p>
                <Time format="yy-MM-dd" at={$message_for_face_all.by_face[o.face_id].date_min} />
              </p>
            {/if}
  ...
  {:else}
    {#each chr_jobs as o (o.face_id)}
      <div
        in:scale={{ delay: 0, duration: 600, opacity: 0, start: 0, easing: backOut }}
