...
for (const key of Object.keys(IconDic).sort()) {
  const Icon = IconDic[key]
  ICONS.push({ key, Icon })
}
</script>
...
<Report handle="SSAY">
  ...
  <article class="text x3">
    {#each FONTS as key}
      <span class="plain">
        <svg class="icon"><use xlink:href="#{key}" /></svg>
      </span>
    {/each}
  </article>
  <br />
...
.spin {
  display: inline-block;
  animation: rotate_anime 4s linear infinite forwards;
}
