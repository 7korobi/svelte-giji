...
export function to_tempo_bare(size, zero, write_at_src) {
...
export function tickTempo(size_str, zero_str = '0s') {
  const size = to_msec(size_str);
  const zero = to_msec(zero_str) + tempo_zero;
  return tickTempoBare(size, zero);
}
export function tickTempoBare(size, zero) {
  let timerID = null;
...
export function tickTempoBy(fns, zero_str = '0s') {
  const zero = to_msec(zero_str) + tempo_zero;
  let now_idx = 1;
  let timerID = null;
  return readable(null, (set) => {
