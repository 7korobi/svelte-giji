<script >import { __BROWSER__, instanceId } from 'svelte-petit-utils';
...
async function byIcon(...args) {
    icons.forEach((o) => {
        ...
        const transform = `translate(${x - 0.5 * width} ${y - 0.5 * (label ? height + labelHeight : height)}) rotate(${roll}, ${0.5 * width}, ${0.5 * height})`;
