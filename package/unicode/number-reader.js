...
function numTable(option, itemStr, scaleStr, bigStr) {
  ...
  option.big ?? (option.big = [subBigs.length, subBigs.length]);
  ...
  function toRegExp(byScale, byBig, items) {}
  ...
  function build(scanner, scales, isStrict = false) {
  ...
  function calc(num, scaleAt, appendix, isTail = 0 === scaleAt) {
    let { join = '' } = option;
    let leftStr;
    const scaleGap = calcScaleGap(scaleAt);
    const scaleSize = scaleBaseSize ** scaleGap;
