...
function onScroll() {
  ...
  state.view.measureOffset(pageTop, clientWidth - width - pageLeft, clientHeight - height - pageTop, pageLeft);
...
function FollowZoom(width, height, scale) {
  if (!isIOS) {
  ...
  if ((width < height && availWidth < availHeight) || (width > height && availWidth > availHeight)) {
  } else {
...
function FollowOrientation(width, height) {
...
function FollowSize(width, height, scale) {
  width *= scale;
  height *= scale;
