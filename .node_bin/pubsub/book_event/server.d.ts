...
export declare const events: {
    $match: (ids: (`${string}-${number}-${number}` | `${string}-${number}-top`)[]) => {
        _id: {
    ...
    live: ($match: any, set: ($set: BookEvent) => Promise<import("mongodb").ModifyResult<BookEvent>>, del: (ids: (`${string}-${number}-${number}` | `${string}-${number}-top`)[]) => Promise<import("mongodb").DeleteResult>) => import("mongodb").ChangeStream<BookEvent>;
