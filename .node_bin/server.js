...
function listen(socketio, models, stores) {
  ...
  for (const name in stores) {
    ...
    const type = [];
    if (model3 && store) {
      if (model3.isLive && model3.live && store.qid)
        type.push("LIVE");
      if (model3.$match && model3.query && store.qid, store.format && store.reduce)
        type.push("READ");
      if (model3.set && model3.del && store.qid)
        type.push("WRITE");
    } else {
      if (!model3)
        type.push("NO-MODEL");
...
var chr_face_default = [
  ...
  },
  {
    _id: "sf046",
    name: "\u30AF\u30EC\u30E1\u30F3\u30B9",
    comment: "\u6075\u6BD4\u5BFF\u9854\u306A\u306E\u3067\u3001\u5BA2\u4EBA\u55B6\u696D\u8077\u3001\u3068\u3044\u3046\u611F\u3058\u3002\u30AF\u30EA\u30B9\u30DE\u30B9\u3068\u540C\u50DA\uFF08\u4E0A\u53F8\uFF1F\uFF09\u306A\u5370\u8C61\u3092\u5C11\u3057\u3070\u304B\u308A\u4ED8\u3051\u305F\u304B\u3063\u305F\u3002",
    order: 80046,
    tag_ids: ["stratos"]
  }
];
...
var Tags = MapReduce({
  ...
  order: (data, { sort: sort3, group_sort: group_sort2 }) => {
    data.group = group_sort2(data.base, (d) => sort3(d).asc((o) => o[0].list[0].order), (d) => sort3(d).asc((o) => o.list[0].order), (d) => {
...
var message_for_face = model2({
  ...
  order(data, { sort: sort3, group_sort: group_sort2 }) {
    data.folder = group_sort2(data.by_folder, (by_folder) => {
      var _a2;
