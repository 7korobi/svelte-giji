...
function modelAsMongoDB(collection, $project) {
  ...
  return {
    ...
    isLive: async () => true,
    live: ($match, set2, del2) => watch(set2, del2, table(), pipeline($match, $project)),
    query: async ($match) => table().aggregate(pipeline($match, $project)).toArray()
  };
}
function exit() {
  client.close();
